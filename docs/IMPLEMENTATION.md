...
> This block is the current resume point and supersedes the dated section below where they conflict.
>
> **Who:** the project director / sole decider is **Hayden Toppeross** (haydentoppeross@gmail.com). Older docs call him **"Jamee" — that was wrong; it always meant Hayden.** A repo-wide `Jamee → Hayden` correction was applied on this branch 2026-06-21. If any file still says "Jamee", it's stale — fix it to Hayden.
>
> **Branch reconciliation DONE + PUSHED 2026-06-21:** the three finished Phase-2 branches (`stage-d-processor-inspector` was already on main; `llama-tool-call-loop` + `phase-2-closeouts`) are **all merged into `main`**, which is **pushed to `origin/main` (`6dc71e7`)**. The merged branches were **deleted** (local + `origin/llama-tool-call-loop`); 13 stale `worktree-agent-*` worktrees were **pruned** (all verified content-merged). The GUI branch `import-anatomy-assets` was **rebased onto the new `main`** and is the only live feature branch — **CURRENT branch, resume here.** Merged main: 365 SPM tests + app target build, green. Conflicts resolved by hand were the `InferenceModelCatalogue` voice-pass × `supportsToolLoop` overlap and the per-branch `IMPLEMENTATION.md` forks. Local safety tags `reconcile-backup-main` / `reconcile-backup-gui` exist (removable). Leftover out-of-scope: stale `origin/feat/phase-2a-inference-engine-abstraction` (long-merged); `main`'s on-disk docs still say "Jamee" until this branch lands.

### Branch map
- **`main`** (pushed, `6dc71e7`) — Phase 2 **complete**: Stages A–D + the llama tool-call execute/iterate loop + the close-outs (voice pass, `DebugBrainView` resolver parity) all merged. Processor inspector + token metering + engine selection/downloads + tool loop all live here.
...
### On-device debugging (2026-06-29) — three fixes, all on `import-anatomy-assets`
First real device runs of the two-mode GUI surfaced three issues; the first two were code bugs (fixed + tested), the third is environmental.
1. **Taps were dead** (`be4c3b1`) — the embedded `SpriteView`'s `SKScene.touchesBegan` never receives touches in this SwiftUI composition (chat face *and* workbench organ ring). Fixed by routing from the SwiftUI gesture layer: chat face `.onTapGesture` → `toggleMode` (**confirmed on device**); workbench `.onTapGesture(coordinateSpace: .local)` → new `AnatomyScene.routeTap(atViewPoint:)` (`convertPoint(fromView:)` + `nodes(at:)`, handles the aspectFit letterbox). **Workbench organ taps not yet device-confirmed** — Hayden to verify (tap Processor → inspector opens).
2. **Chat "froze"/killed the app** (`d40b71c`) — `@MainActor` `UsageListener`/`ToolInvocationListener` `.sink` invoked off-main from the `ConversationManager`/`HeartbeatManager` **actor** executor → Swift concurrency **SIGTRAP** on every turn (the token-usage event). Fixed with `.receive(on: DispatchQueue.main)`. Reproduced + regression-guarded by `ChatRespondPipelineTests` (it SIGTRAPed before the fix). Latent because unit tests `.send` on main; never exercised on device pre-two-mode.
3. **`loaded 0 modules` + "laggy from launch"** — *not* code bugs. `0 modules` was **stale provisioning** (a fresh sim install loads all 3); now self-healing via the bundle-sync below. The lag is **device-perf**: the app idles at **0% CPU** on the simulator and is responsive — the device is FM-`deviceNotEligible` (older) on a **Debug** build. Advise a Release build before treating lag as a defect; a future "throttle continuously-rendering SpriteViews on weak devices" pass is noted but unprofiled (can't repro on the fast sim). Lesson recorded: a green Combine unit test that `.send`s on main gives false confidence for code that publishes from an actor.

### BotProvisioner additive bundle-sync (2026-06-29, `4d5be70` + `c725428`)
- New `BotProvisioner.syncMissingFiles(from:into:)` copies bundled `default-bot/` files **missing** from the active bot dir on every launch (so app updates that add modules/assets reach existing installs), creating intermediate dirs, **never overwriting user edits**. Wired into both existing-dir paths of `ensureDefaultBotProvisioned`; fresh-install `copyItem` path unchanged. **Scope: additive only** — does not update changed-upstream files the user already holds; a user-deleted shipped file reappears (accepted trade-off). Spec/plan: `docs/plans/botprovisioner-bundle-sync.md`. Caught a macOS `/var`→`/private/var` symlink subtlety via `resolvingSymlinksInPath()` (no-op on iOS). **395 SPM tests** green; app builds.
...
- Organ **column/grille positions** are screenshot-tuned approximations; organ **glyph picks** are the v01 §4 choices — both want Hayden's exact eyeball.
...
- **Stage C** (`docs/plans/phase-2c-engine-selection-and-models.md`) — **C1** (variable-window budgeting) + **C2** (`identity/processor.md` + `CapabilityDetector`) + **C3** (catalogue + download manager + lifecycle) + **C4** (`b0tApp` engine-selection wiring) **ALL DONE (C3/C4 2026-06-05).** C3: `InferenceModelCatalogue` (b0tBrain, verified rows), `ModelDownloadManager` + `ModelStore` (b0tLlama). C4: `EngineSelector` (b0tCore) + `b0tApp.resolveClient` (processor.md → capability + model-presence gate → FM/Llama/fallback). **Stage D and the llama tool-call execute/iterate loop are DONE** — Stage D on `stage-d-processor-inspector`, tool loop on `llama-tool-call-loop`. Only the device RAM/latency confirmation remains.
- **§14 Q6 — desk half DONE (2026-06-02), device half pending Hayden.** Research + candidate matrix complete; trio **locked**: default **Qwen3-1.7B** (Apache-2.0), opt-in **Llama 3.2 1B** ("Built with Llama"), third **Qwen2.5-1.5B-Instruct** (Apache-2.0); all Q4_K_M. SmolLM2 disqualified (27% BFCL) → stays test-fixture only. Tool-calling settled by [ADR-0018](decisions/0018-llama-tool-calling-via-gbnf-pure-c.md): **GBNF harness on the pure-C boundary** (minja/`common` confirmed NOT in the b9415 xcframework; pure-C principle kept). Matrix + RAM math + **turnkey on-device validation protocol** in `docs/specs/phase-2c-q6-model-lineup-validation.md`. **Validation harness BUILT (2026-06-05):** `Q6ValidationView` + `Q6Runner` (`b0tApp/Sources/Debug/`, DEBUG-only — long-press home → debug brain → **Q6**), on a real GBNF tool-call loop (`LlamaToolCallLoop` + `ToolCallGrammarBuilder` in `b0tLlama`, host-tested in `b0tLlamaTests`, gated e2e in `Q6ToolCallLiveTests`). **Functional half DONE on host (2026-06-05)** — gated `Q6HostFunctionalTests` ran the real trio on the macOS slice: **all three PASS the template gate (go/no-go), GBNF 5/5, tool-call 88–100%** (floor check; see spec §6a). Pinned SHAs + SHA-256 + sizes **captured & verified** (spec §4) → C3 catalogue rows ready. **Remaining device-only:** RAM fit on the 6GB floor + A15 latency (spec §6b) — low-risk, non-blocking. **C3/C4 now DONE (2026-06-05)** — see the Stage C bullet above. **Next (remote-friendly):** Stage D (Processor inspector UI + token gauge + download-progress UI) and/or the tool-call execute/iterate loop; the iPhone 13 Pro RAM/latency pass remains the only device-gated item.
- **Stage D — DONE on branch `stage-d-processor-inspector` (2026-06-05).** Processor inspector (3-tab Controls/Directory/.md), per-turn token metering (crown + Controls gauge), interactive model switching, and model downloads — all built and wired. Spec: `docs/specs/phase-2-stage-d-processor-inspector.md`; plan: `docs/plans/phase-2d-processor-inspector.md` (16 TDD tasks, subagent-driven). Architecture (approach A): a shared `EngineHost` (b0tLlama) wraps the live engine so switching a model in the inspector swaps the engine chat + heartbeat use; `ContextAssembler` reads the context window live; `GenerationUsage` is emitted per turn/beat on `usageEvents` and bridged to the crown/gauge via `UsageListener`; the download/switch UI binds to dependency-inverted seams (`ModelDownloadServicing`/`ProcessorControlling` in b0tHome, conformers `AppModelDownloadService`/`AppProcessorController` + the shared `ProcessorRuntime` in b0tApp), keeping b0tHome off the llama binary. **Deferred (noted in the spec §9):** live per-token metering (would reopen the Stage A protocol), the temperature slider (temp still hardcoded), `DebugBrainView` resolver parity, and the storage line's total-capacity figure (v1 placeholder `max(free, 13 GB)`). **Pending Hayden:** the simulator visual smoke (RenderPreview/§14 walk — agent harness can't drive the simulator UI deterministically, per the Phase 3/4 convention). **The iPhone 13 Pro RAM/latency pass (spec §6b) is now the only device-gated Phase-2 item.**
- **Llama tool-call execute/iterate loop — DONE (branch `llama-tool-call-loop`, 2026-06-05).** Spec: `docs/specs/phase-2-llama-tool-call-loop.md`; plan: `docs/plans/phase-2-llama-tool-call-loop.md`. As-built shape: two-pass `LlamaEngine.generate` (gate→execute→answer). Pass 1 generates under a GBNF ToolGate grammar to pick a tool or "none"; when a tool is picked, `ToolExecutor` (existential-opening via `LlamaGenerating`-seam refactor) calls the concrete `Tool.call(arguments:)`, records a `ToolCallRecord`, and injects the result into the user prompt; pass 2 generates the final `StructuredOutput` answer grounded on the result. Gate is `supportsToolLoop: Bool` (init-time; curated per `InferenceModelCatalogue` row). Serves both chat and heartbeat via the `InferenceEngine` contract. **Deferred:** multi-step tool chains (more than one tool per turn); runtime `supportsToolLoop` auto-detection (curated only for now). Gated live test added to `b0tLlamaLiveTests` (`LIVE_LLAMA=1`), mirroring the existing `LlamaModelCache.ensureModel()` / SmolLM2 pattern.
...
**Test/build:** 363 SPM tests (9 `LIVE_LLAMA`/`Q6_HOST`-gated), 0 failures; iOS app builds clean (`-warnings-as-errors`). (331 → 352 Stage D → 363 tool-call loop.) **Phase 2 status:** Stages A–D, the llama **tool-call execute/iterate loop**, and the two small close-outs (model **disclosure-copy voice pass** + **`DebugBrainView` resolver parity**) are **ALL MERGED TO MAIN (2026-06-21 reconciliation)** — formerly on branches `stage-d-processor-inspector` / `llama-tool-call-loop` / `phase-2-closeouts`. Still remaining (deferred-by-design): **live per-token metering** + the **temperature slider** (both from Stage D). The **only device-gated item** is the iPhone 13 Pro RAM/latency confirmation (spec §6b) — low-risk, non-blocking. (GUI revision continues on branch `import-anatomy-assets`, now rebased onto this main, pending Hayden's visual pass + the lower-section tabs design question.) **Known flake:** `b0tHomeTests`/`OrganInspectionViewTests` intermittently segfaults (signal 11) under parallel runs — pre-existing, clean in isolation; worth a dedicated fix.
...
- **§14 Q1/Q2 resolved 2026-05-30/31** against Hayden's home-screen mockup
...
- **Layout (captured 2026-05-31):** organ ring reorganized — left/right
  ...
  (fancy eye-screen → v2). See ADR-0014/0016. Memory-icon swap pending Hayden's
  asset filenames.
...
- Manual simulator launch (the plan's Step 22.4 / "see 'active: b0t-01' on screen") deferred to Hayden — agent harness can't drive the simulator UI deterministically.
...
- Manual smoke checklist (real-device or simulator with Apple Intelligence enabled) deferred to Hayden — agent harness can't drive the simulator UI deterministically. Spec §10 acceptance criteria #1–#6 (in-conversation calendar/reminders/health flow) are the load-bearing live verification.

### Manual smoke verification (2026-05-05)

Hayden smoke-tested on iPhone 17 Pro simulator (iOS 26.3). End-to-end behaviour confirmed:
...
- ~~**`BotProvisioner` only copies once**~~ **RESOLVED 2026-06-29** (`4d5be70`+`c725428`): added additive bundle-sync on every launch (`syncMissingFiles`) — copies missing bundled files into the active bot, never overwriting user edits. See the 2026-06-29 handoff section above.
...
- **Hilfer's Part PNGs + organ icons + module sub-icons + file icon** still pending Hayden delivery from Gamelabs Studio. `assets/face-parts/placeholder/Wundercog/Hilfer/` holds placeholder squares; `b0tApp/Resources/Assets.xcassets/` is empty for the Hilfer Parts and organs. Slice 10 visual sign-off (T61) blocks ledger flip from "code complete" to "complete."
- ~~**`BotProvisioner` once-only-on-first-launch behaviour**~~ **RESOLVED 2026-06-29** — additive bundle-sync on launch now propagates new bundled files (modules, assets) to existing installs without clobbering user edits (`4d5be70`+`c725428`; see the 2026-06-29 handoff section).
...
### Manual smoke verification (deferred — pending Hayden)
